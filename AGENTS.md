...
1c. **Find the first-keypress timestamp before/after the run, and verify the MIDI starts there.** Auto-detection (`--start-time` omitted) is usually right but not always — the per-frame face-zone scanner can be fooled by intro overlays whose colors land in the saturation band. Procedure: step `ffmpeg -ss <t> -i ... -frames:v 1` at ~0.25s intervals starting from t=0 until the first key glow appears under the keyboard's red line. Then dump `mid.tracks[*]` first events and confirm `MIDI t=0 ≈ video t=<first_keypress>`. If they disagree by more than ~50ms, override with `--start-time <found_t>`. Do not paraphrase "the script auto-detected start" — actually inspect frames. Reading visually: cyan/blue glow on a black key is a black-key press; look at where the lit column sits *relative to the labeled C-marks* on the keyboard, count white keys.
1d. **Don't claim a note is misplaced without cropping.** When the MIDI says E3 and you think the lit key is C#3 (or similar), crop a small region of the frame around the keyboard and inspect: a black-key press shows a *narrow* glow centered on a black key; a white-key press shows a *wider* glow on a white key bracketed by two black keys. Mistaking a white-key glow for a black-key glow because the surrounding image is busy is a common error — see history 2026-05-06.
