...
1. **Always verify the final output before giving it to the user.** After generating a MIDI file, compare it against the verified ground truth (if one exists) and list ALL differences. Don't just check note names — check the *quantized* output that the user will actually hear/see. **If there are known bugs, list them BEFORE giving the file path.** Never make the user discover bugs you already know about. (See `.agents/error-log.md` 2026-03-20, 2026-03-29, 2026-03-30 entries.)
